...
Abzeichen sind fesselnde Symbole, die neben einem Element angezeigt werden!

**_Beispielbild:_** (füge hier ein Bild ein)

Um Abzeichen auf einem Element zu verwenden, kodiert ihr es einfach wie folgt:

```lua
local item = NativeUI.CreateItem('Element-Name', 'Element-Beschreibung')
item:SetRightBadge(ID)
```

Die 'ID' repräsentiert das spezifische Symbol, das das Abzeichen anzeigen soll. Schaut euch die aktuellen verfügbaren IDs an:
...
Zusätzlich erlauben bestimmte Elemente (nicht alle) Abzeichen auf der linken Seite mit `SetLeftBadge` zu platzieren. Wenn das linke Abzeichen nicht anwendbar ist, wird NativeUI freundlich darauf hinweisen, dass es nicht möglich ist. Lasst also eurer Kreativität freien Lauf mit Abzeichen auf euren Elementen! 😊

[Quelle](https://github.com/MrMathias154/NativeUILua-Reloaded/blob/master/elements/Badge.lua)
# Inhalt der Datei:
```lua
BadgeStyle = {
	None = 0,
