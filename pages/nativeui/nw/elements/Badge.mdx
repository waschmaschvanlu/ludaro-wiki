...
Badges are captivating symbols displayed alongside an element!

**_Example Image:_** (insert an image here)

To use badges on an element, you simply encode it as follows:

```lua
local item = NativeUI.CreateItem('Element Name', 'Element Description')
item:SetRightBadge(ID)
```

The 'ID' represents the specific symbol that the badge should display. Check out the currently available IDs:
...
Additionally, specific elements (not all) allow badges to be placed on the left side using `SetLeftBadge`. If the left badge is not applicable, NativeUI will kindly notify you that it's not possible. So go ahead and let your creativity run wild with badges on your elements! 😊

[Source](https://github.com/MrMathias154/NativeUILua-Reloaded/blob/master/elements/Badge.lua)
```lua
BadgeStyle = {
	None = 0,
	BronzeMedal = 1,
